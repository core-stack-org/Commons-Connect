...
const MapComponent = () => {
      ...
      mapRef.current.on("click", async (e) => {
        ...
        if (croppingFeature !== undefined) {
          ...
          MainStore.setSelectedMwsDrought(
            src.find((f) => f.get("uid") === croppingFeature.values_.uid)
              ?.values_ ?? null,
          );
        }
      });
  ...
  const fetchResourcesLayers = async () => {
    ...
    const wellLayer = await getVectorLayers(
      "resources",
      "well_" + currentPlan.plan_id + "_" + `${districtName}_${blockName}`,
      true,
      true,
    );
    ...
    const waterStructureLayer = await getVectorLayers(
      "resources",
      "waterbody_" + currentPlan.plan_id + "_" + `${districtName}_${blockName}`,
      true,
      true,
    );
    ...
    if (currentScreen === "HomeScreen") {
      ...
      try {
        ...
        assetsLayerRefs[2].current.setStyle(function (feature) {
          if (
            shouldShowWaterStructure(feature.get("wbs_type"), "Groundwater")
          ) {
      ...
      try {
        ...
        assetsLayerRefs[2].current.setStyle(function (feature) {
          if (
            shouldShowWaterStructure(feature.get("wbs_type"), "SurfaceWater")
          ) {
            return getWaterStructureStyle(feature);
          }
          return null;
      ...
      try {
        ...
        assetsLayerRefs[2].current.setStyle(function (feature) {
          if (
            shouldShowWaterStructure(feature.get("wbs_type"), "Agriculture")
          ) {
            return getWaterStructureStyle(feature);
          }
          return null;
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (GpsLayerRef.current) {
        ...
        GpsLayerRef.current.setMap(null);
  ...
  // Handle Agrohorticulture Terrain Mask toggle - Add mask overlay on top
  useEffect(() => {
    // Debug logging
    console.log("Agrohorticulture Mask Effect:", {
      currentScreen,
      maskLayerExists: AgrohorticulureRefs[1].current !== null,
      isActive: MainStore.isAgrohorticultureMaskActive,
    });

    if (
      currentScreen !== "Agrohorticulture" ||
      AgrohorticulureRefs[1].current === null
    ) {
      return;
    }

    // Get the mask layer
    const maskLayer = AgrohorticulureRefs[1].current;

    // Show or hide the mask layer based on the toggle state
    if (MainStore.isAgrohorticultureMaskActive) {
      // Add mask layer if not already on map
      if (!mapRef.current.getLayers().getArray().includes(maskLayer)) {
        mapRef.current.addLayer(maskLayer);
        console.log("Added Agrohorticulture mask layer");
      }
    } else {
      // Remove mask layer if it's on the map
      if (mapRef.current.getLayers().getArray().includes(maskLayer)) {
        mapRef.current.removeLayer(maskLayer);
        console.log("Removed Agrohorticulture mask layer");
      }
    }
  }, [MainStore.isAgrohorticultureMaskActive, currentScreen]);
