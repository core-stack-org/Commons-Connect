export default function getOdkUrlForScreen(screen, step, latlong, settlement_name, settlement_id, block_name, plan_id, plan_name, crop_id, maintain = false, userLatLong){
    ...
    if(screen === "Resource_mapping" && step === 0){
        ...
        odk_url = `${import.meta.env.VITE_ODK_CROP_GRID}$smauFD3` + "&d[/data/hamlet_id]=" + settlement_id + "&d[/data/crop_Grid_id]=" + crop_id + "&d[/data/beneficiary_settlement]=" + settlement_name + "&d[/data/plan_id]=" + plan_id + "&d[/data/user_latlon]=" + userLatLong[1].toString() + ", " + userLatLong[0].toString() + "&d[/data/plan_name]=" + plan_name + "&d[/data/meta/instanceID]="
    }

    else if(screen === "Groundwater" && step === 1 && !maintain){
        odk_url = `${import.meta.env.VITE_ODK_GROUNDWATER_BUILD_RECHARGE}$seU5QjKkgGjdav$pga66iH972V93scb8BxwvuBuQVxIUxLo$gf8F` + "&d[/data/GPS_point/point_mapsappearance]=" + latlong[1].toString() + "%20" + latlong[0].toString() + "&d[/data/block_name]=" + block_name + "&d[/data/plan_id]=" + plan_id + "&d[/data/user_latlon]=" + userLatLong[1].toString() + ", " + userLatLong[0].toString() + "&d[/data/plan_name]=" + plan_name + "&d[/data/corresponding_work_id]=" + "007101" + "&d[/data/meta/instanceID]="
    }

    else if(screen === "Groundwater" && step === 1 && maintain){
        odk_url = `${import.meta.env.VITE_ODK_GROUNDWATER_PROVIDE_MAINTAIN}`+ "&d[/data/GPS_point/point_mapsappearance]=" + latlong[1].toString() + "%20" + latlong[0].toString() + "&d[/data/block_name]=" + block_name + "&d[/data/plan_id]=" + plan_id + "&d[/data/plan_name]=" + plan_name + "&d[/data/user_latlon]=" + userLatLong[1].toString() + ", " + userLatLong[0].toString() + "&d[/data/corresponding_work_id]=" + "0001829" + "&d[/data/meta/instanceID]="
    }
